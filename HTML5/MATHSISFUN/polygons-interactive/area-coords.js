var w,h,ratio,i,s,el,g,div,dragQ,game,my={};function areacoordsMain(imode){my.version='0.89'
mode=typeof imode!=='undefined'?imode:5;console.log("mode",mode);this.modePts=5;if(isNaN(mode)){}else{this.modePts=Number(mode);}
...
function toggleRegular(){regQ=!regQ;toggleBtn("regBtn",regQ);update();}
...
function ontouchend(evt){theCanvas.addEventListener('touchstart',ontouchstart,false);window.removeEventListener("touchend",ontouchend,false);if(dragQ){dragQ=false;window.removeEventListener("touchmove",ontouchmove,false);}}
function dopointer(e){var bRect=theCanvas.getBoundingClientRect();mouseX=(e.clientX-bRect.left)*(el.width/ratio/bRect.width);mouseY=(e.clientY-bRect.top)*(el.height/ratio/bRect.height);var overQ=false;for(i=0;i<pts.length;i++){if(hitTest(pts[i],mouseX,mouseY)){overQ=true;dragNo=i;}}
if(overQ){document.body.style.cursor="pointer";}else{document.body.style.cursor="default";}}
function domousemove(e){document.body.style.cursor="default";var bRect=theCanvas.getBoundingClientRect();mouseX=(e.clientX-bRect.left)*(el.width/ratio/bRect.width);mouseY=(e.clientY-bRect.top)*(el.height/ratio/bRect.height);for(i=0;i<pts.length;i++){if(hitTest(pts[i],mouseX,mouseY)){dragNo=i;document.body.style.cursor="pointer";}}}
function mouseDownListener(evt){var i;var highestIndex=-1;var bRect=theCanvas.getBoundingClientRect();mouseX=(evt.clientX-bRect.left)*(el.width/ratio/bRect.width);mouseY=(evt.clientY-bRect.top)*(el.height/ratio/bRect.height);downX=mouseX;downY=mouseY;prevX=mouseX;prevY=mouseY;dragQ=false;dragScreenQ=false;for(i=0;i<pts.length;i++){if(hitTest(pts[i],mouseX,mouseY)){dragNo=i;dragQ=true;if(i>highestIndex){dragHoldX=mouseX-pts[i].x;dragHoldY=mouseY-pts[i].y;highestIndex=i;dragNo=i;}}}
...
doType();}
function doType(){g.clearRect(0,0,el.width,el.height);this.graph.drawGraph();if(regQ){poly.makeRegular();poly2shapes();}
drawPts();var descr=['',''];if(mode=='coords'){}else{descr=getDescr();}
...
return pts;}
function makeShapes(){var i;var tempX;var tempY;var tempColor;var pos=getRegular(4,3,3,1.2,this.modePts);shapes=[];pts=[]
for(i=0;i<this.modePts;i++){shapes.push(pos[i]);var pt=new Pt(coords.toXPix(shapes[i].x),coords.toYPix(shapes[i].y));pts.push(pt);}
console.log("makeShapes",this.modePts,shapes,poly.pts);}
function drawPts(){var i;g.strokeStyle="rgba(0, 0, 255, 0.5)";g.fillStyle="rgba(255, 255, 100, 0.1)";pts=[];for(i=0;i<shapes.length;i++){var pt=new Pt(coords.toXPix(shapes[i].x),coords.toYPix(shapes[i].y));pts.push(pt);}
var dbg="";for(i=0;i<pts.length;i++){var pt=pts[i];g.fillStyle="rgba(0, 0, 255, 0.3)";g.beginPath();g.arc(pt.x,pt.y,pt.rad,0,2*Math.PI,false);g.closePath();g.fill();g.fillStyle="rgba(0, 0, 0, 0.8)";g.beginPath();g.arc(pt.x,pt.y,2,0,2*Math.PI,false);g.closePath();g.fill();g.textAlign="left";if(coordsQ){g.font="bold 14px Arial";var txt='(';txt+=shapes[i].x.toFixed(1);txt+=',';txt+=shapes[i].y.toFixed(1);txt+=')';g.fillText(txt,pt.x+5,pt.y-9);}else{g.font="14px Arial";g.fillText(String.fromCharCode(65+i),shapes[i].x+5,shapes[i].y-9);}
dbg+='['+Math.floor(shapes[i].x)+","+Math.floor(shapes[i].y)+"],";}
poly.pts=pts;poly.updateMe();g.strokeStyle="rgba(255, 0, 0, 0.5)";g.lineWidth=1.5;if(guidesQ){var orig={x:coords.toXPix(0),y:coords.toYPix(0)};poly.drawGuides(g,orig);}
g.strokeStyle="rgba(0, 0, 255, 0.5)";g.fillStyle="rgba(255, 255, 100, 0.1)";g.lineWidth=1;drawShapeQ=true;if(mode=='coords'){drawShapeQ=sidesQ;}
if(drawShapeQ)poly.drawLines(g);if(anglesQ)poly.drawAngles(g);if(diagsQ)poly.drawDiags(g);if(sidesQ)poly.drawSides(g);}
function Poly(){this.pts=[];}
Poly.prototype.updateMe=function(){setAngles(this.pts);sides=getSides(this.pts);};Poly.prototype.drawDiags=function(g){g.strokeStyle="#666666";var diagCount=0;for(var i=0;i<this.pts.length-2;i++){for(var j=i+2;j<this.pts.length;j++){if(i==0&&j==this.pts.length-1){}else{g.beginPath();g.moveTo(this.pts[i].x,this.pts[i].y);g.lineTo(this.pts[j].x,this.pts[j].y);g.stroke();diagCount++;}}}
console.log("diagCount="+diagCount);};Poly.prototype.drawLines=function(g){g.beginPath()
for(var i=0;i<this.pts.length;i++){g.lineTo(pts[i].x,pts[i].y);}
g.closePath()
g.fill();g.stroke();};Poly.prototype.drawGuides=function(g,orig){var ptsLen=this.pts.length;for(var i=0;i<ptsLen;i++){var pt=pts[i];g.beginPath();g.strokeStyle="rgba(0, 0, 0, 0.5)";g.moveTo(orig.x,pt.y);g.lineTo(pt.x,pt.y);g.stroke();g.beginPath();g.moveTo(pt.x,pt.y);g.lineTo(pt.x,orig.y);g.stroke();}};Poly.prototype.drawSides=function(g){var ptC=new Pt();ptC.setAvg(this.pts);g.fillStyle="#000000";g.font="bold 12px Arial";var ptM=new Pt();var ptsLen=this.pts.length;for(var i=0;i<ptsLen;i++){var pt=this.pts[i];ptM.setAvg([this.pts[i],this.pts[loop(i,0,ptsLen-1,1)]]);ptM.interpolate(ptM,ptC,1.2);var side=sides[loop(i-1,0,ptsLen-1,1)];side=(coords.xScale*side).toFixed(2);g.fillText(side,ptM.x-10,ptM.y+5,100);}};Poly.prototype.isConcave=function(){for(var i=0;i<this.pts.length;i++){var pt=this.pts[i];var angDeg=Math.round(pt.getAngle()*180/Math.PI);if(angDeg>180)return true}
return false}
Poly.prototype.drawAngles=function(g){var angSum=0;var angDescr="";var angs=[];for(var i=0;i<this.pts.length;i++){var pt=this.pts[i];var angDeg=Math.round(pt.getAngle()*180/Math.PI);var d=30;if(angDeg==90){g.strokeStyle='#888888';g.lineWidth=2;g.drawBox(pt.x,pt.y,25,pt.angleOut-Math.PI/2);}else{if(angDeg>90){g.strokeStyle='#ff0000';g.lineWidth=2;d=Math.max(20,30-(angDeg-90)/6);}else{g.strokeStyle='#4444FF';g.lineWidth=2;}
...
var aMid=(pt.angleIn+pt.angleOut)/2;var txtPt=new Pt(0,0)
...
return angs};Poly.prototype.userAngle=function(x){return Math.round(x*180/Math.PI,this.dec);};Poly.prototype.getArea=function(){var a=0;for(var i=0;i<this.pts.length;i++){var pt0=this.pts[i];var pt1=this.pts[loop(i,0,this.pts.length-1,1)];var a1=(pt0.x*pt1.y-pt0.y*pt1.x);a+=a1;}
a=Math.abs(a)/2;return a;};Poly.prototype.getDiagCount=function(){var n=this.pts.length;return n*(n-3)/2;};Poly.prototype.isComplex=function(){var lns=[];for(var i=0;i<this.pts.length;i++){lns.push(new Line(this.pts[i],this.pts[loop(i,0,this.pts.length-1,1)]));}
...
return sameQ;};Poly.prototype.makeRegular=function(){var ptC=new Pt();ptC.setAvg(this.pts);var rad=1
var avgQ=false
if(avgQ){var rads=[];for(var i=0;i<this.pts.length;i++){var pt=this.pts[i];rads[i]=dist(pt.x-ptC.x,pt.y-ptC.y);}
rad=avg(rads);}else{if(dragNo<this.pts.length){pt=this.pts[dragNo]}else{pt=this.pts[0]}
rad=dist(pt.x-ptC.x,pt.y-ptC.y);}
var sttAngle=Math.atan2(pts[0].y-ptC.y,pts[0].x-ptC.x);var dAngle=Math.PI*2/this.pts.length;for(i=0;i<this.pts.length;i++){var angle=sttAngle+i*dAngle;var pt=this.pts[i];pt.x=ptC.x+rad*Math.cos(angle);pt.y=ptC.y+rad*Math.sin(angle);}};function Pt(ix,iy){this.x=ix;this.y=iy;this.rad=9;this.color="rgb("+0+","+0+","+255+")";var prevx;var prevy;var a;var prevQ=false;var validPtQ;angleIn=0;angleOut=0;}
Pt.prototype.setxy=function(ix,iy){this.x=ix;this.y=iy;validPtQ=true;};Pt.prototype.setPrevPt=function(){prevx=this.x;prevy=this.y;prevQ=true;};Pt.prototype.getAngle=function(){return this.angleOut-this.angleIn;};Pt.prototype.drawMe=function(g){g.fillStyle="rgba(0, 0, 255, 0.3)";g.beginPath();g.arc(this.x,this.y,20,0,2*Math.PI,false);g.closePath();g.fill();};Pt.prototype.getAvg=function(pts){var xSum=0;var ySum=0;for(var i=0;i<pts.length;i++){xSum+=pts[i].x;ySum+=pts[i].y;}
newPt=new Pt(xSum/pts.length,ySum/pts.length);newPt.x=xSum/pts.length;newPt.y=ySum/pts.length;return newPt;};Pt.prototype.setAvg=function(pts){this.setPrevPt();var newPt=this.getAvg(pts);this.x=newPt.x;this.y=newPt.y;validPtQ=true;};Pt.prototype.interpolate=function(pt1,pt2,f){this.setPrevPt();this.x=pt1.x*f+pt2.x*(1-f);this.y=pt1.y*f+pt2.y*(1-f);validPtQ=true;};function isNear(a,b,toler){if(Math.abs(a-b)<=toler){return true;}else{return false;}}
...
return ip;};Line.prototype.getMidPt=function(){return new Pt((this.a.x+this.b.x)/2,(this.a.y+this.b.y)/2);};Line.prototype.getClosestPoint=function(toPt,inSegmentQ){var AP=toPt.translate(this.a,false);var AB=this.b.translate(this.a,false);var ab2=AB.x*AB.x+AB.y*AB.y;var ap_ab=AP.x*AB.x+AP.y*AB.y;var t=ap_ab/ab2;if(inSegmentQ){t=constrain(0,t,1);}
closest=this.a.translate(AB.multiply(t));return closest;};Line.prototype.setLen=function(newLen,fromMidQ){fromMidQ=typeof fromMidQ!=='undefined'?fromMidQ:true;var len=this.getLength();if(fromMidQ){var midPt=this.getMidPt();var halfPt=new Pt(this.a.x-midPt.x,this.a.y-midPt.y);halfPt.multiplyMe(newLen/len);pt1=midPt.translate(halfPt);pt2=midPt.translate(halfPt,false);}else{var diffPt=new Pt(this.a.x-this.b.x,this.a.y-this.b.y);diffPt.multiplyMe(newLen/len);pt2=pt1.translate(diffPt,false);}};Line.prototype.getAngle=function(){return Math.atan2(this.b.y-this.a.y,this.b.x-this.a.x);};function Coords(left,top,width,height,xStt,yStt,xEnd,yEnd,uniScaleQ){this.left=left;this.top=top;this.width=width;this.height=height;this.xStt=xStt;this.yStt=yStt;this.xEnd=xEnd;this.yEnd=yEnd;this.uniScaleQ=uniScaleQ;this.xLogQ=false;this.yLogQ=false;this.xScale;var xLogScale;this.yScale;this.calcScale();}
