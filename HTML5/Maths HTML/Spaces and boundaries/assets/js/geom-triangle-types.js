...
function drawShapes() {
    ...
    g.moveTo(shapes[numShapes - 1].x, shapes[numShapes - 1].y);
    for (i = 0; i < numShapes; i++) {
    ...
    for (i = 0; i < numShapes; i++) {
        ...
        g.beginPath();
        g.arc(shapes[i].x, shapes[i].y, 8, 0, 2 * Math.PI, false);
        g.closePath();
        g.fill();
    ...
    if (my.mode == 'circum') {
        ...
        drawDot(mid.x, mid.y);
        var radLn = new Line(mid, new Pt(shapes[0].x, shapes[0].y));
...
Triangle.prototype.drawSides = function(g) {
    ...
    var ptM = new Pt();
    for (var i = 0; i < 3; i++) {
        ptM.setAvg([pts[i], pts[loop(i, 0, 2, 1)]]);
        ptM.interpolate(ptM, ptC, 1.4);
        var side = Math.round(this.getVal(String.fromCharCode(97 + loop(i + 1, 0, 2, 1))));
...
function drawPerpBisectors() {
    var lns = [];
    for (var i = 0; i < 3; i++) {
        var ln = new Line(pts[i], pts[loop(i, 0, 2, 1)]);
        var perpLn = ln.clone();
...
function drawMedians() {
    ...
    for (var i = 0; i < 3; i++) {
        var ln = new Line(pts[i], pts[loop(i, 0, 2, 1)]);
        var midPt = ln.getMidPt();
        var medianLn = new Line(midPt, pts[loop(i + 1, 0, 2, 1)]);
        lns.push(medianLn);
        g.strokeStyle = 'black';
        g.lineWidth = 1;
        g.beginPath();
...
Triangle.prototype.getAltitude = function(ptNo) {
    var aa = pts[loop(ptNo, 0, 2, 1)];
    var ln = new Line(aa, pts[loop(ptNo, 0, 2, 2)]);
...
Triangle.prototype.drawAltitude = function(r, ptNo, drawAsSegmentQ, showTextQ) {
    ...
    var ln = new Line(pts[loop(ptNo, 0, 2, 1)], pts[loop(ptNo, 0, 2, 2)]);
    var iPt = ln.getClosestPoint(pts[ptNo]);
    var htLn = new Line(pts[ptNo], iPt);
...
Triangle.prototype.makeScalene = function(PointNum) {
    ...
    do {
        ...
        if (!scaleneQ) {
            ...
            shapes[pushNum].x += getRandom(-pushDist, pushDist);
            shapes[pushNum].y += getRandom(-pushDist, pushDist);
            this.updateMe();
            shovedQ = true;
        }
        if (shovedQ) {
...
Triangle.prototype.makeIsosceles = function(PointNum) {
    ...
    do {
        ...
        var pushDist = 5;
        if (doubledAng == 90) {
            shapes[0].x += getRandom(-pushDist, pushDist);
            shapes[0].y += getRandom(-pushDist, pushDist);
            shovedQ = true;
        }
        if (doubledAng == 0) {
            shapes[0].x += getRandom(-pushDist, pushDist);
...
Triangle.prototype.makeRight = function(PointNum) {
    ...
    if (Math.round(pts[0].getAngle() * 100) != 157) {
        ...
        var AC = dist(Cobj.x - Aobj.x, Cobj.y - Aobj.y);
...
Triangle.prototype.makeAcute = function(PointNum) {
    ...
    for (var i = 0; i < 3; i++) {
        if (pts[i].getAngle() * 180 / Math.PI > 89) {
            var Aobj = shapes[i];
            var Bobj = shapes[(i + 1) % 3];
            var Cobj = shapes[(i + 2) % 3];
            var Angle = Math.atan2(Aobj.y - Bobj.y, Aobj.x - Bobj.x);
            var AC = dist(Cobj.x - Aobj.x, Cobj.y - Aobj.y);
